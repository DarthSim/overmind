...
func setupStartCmd() cli.Command {
	...
	return cli.Command{
		...
		Flags: []cli.Flag{
			...
			cli.StringFlag{Name: "restart-signals, j", EnvVar: "OVERMIND_RESTART_SIGNALS", Usage: "Specify a signal that will be sent to each process when Overmind will try to restart them. The value passed in should be in the format process=signal,process=signal. Supported signals are: ABRT, INT, KILL, QUIT, STOP, TERM, USR1, USR2"},
